...
class TensorVMEncoding(Encoding):
    ...
    def forward(self, in_tensor: Float[Tensor, "*bs input_dim"], detach=False) -> Float[Tensor, "*bs output_dim"]:
        ...
        if detach:
            plane_coord = plane_coord.detach()
            line_coord = line_coord.detach()
...
class KPlanesEncoding(Encoding):
    ...
    def __init__(
        ...
        self.plane_coefs_mipmap = nn.ParameterList()
        for li in range(self.num_levels):
            ...
            for coo_comb in self.coo_combs:
                ...
                plane_coefs.append(new_plane_coef)
            self.plane_coefs_mipmap.append(plane_coefs)
    ...
    def forward(self, in_tensor: Float[Tensor, "*bs input_dim"], detach=False) -> Float[Tensor, "*bs output_dim"]:
        ...
        for li in range(self.num_levels):
            ...
            for ci, coo_comb in enumerate(self.coo_combs):
                grid = self.plane_coefs_mipmap[li][ci].unsqueeze(0)  # [1, feature_dim, reso1, reso2]
                coords = in_tensor[..., coo_comb].view(1, 1, -1, 2).clip(-1, 1)  # [1, 1, flattened_bs, 2]
                coords = torch.remainder(torch.abs(coords) * (2 ** li), 1) * torch.sign(coords)

                if self.use_hash:
                    coords = coords.view(-1,2)
                    coords = (coords + 1) / 2
                    coords[...,0] *= grid.shape[-2]
                    coords[...,1] *= grid.shape[-1]

                    lefttop = coords.clone().long()
                    righttop = coords.clone().long()
                    leftbottom = coords.clone().long()
                    rightbottom = coords.clone().long()
                    righttop[...,0] += 1
                    leftbottom[...,1] += 1
                    rightbottom += 1
                    alpha_x = coords[...,0] - torch.floor(coords[...,0])
                    alpha_y = coords[...,1] - torch.floor(coords[...,1])
                    ...
                    lefttop[...,0] = xorshift(lefttop[...,0]) % grid.shape[-2]
                    lefttop[...,1] = xorshift(lefttop[...,1]) % grid.shape[-1]
                    righttop[...,0] = xorshift(righttop[...,0]) % grid.shape[-2]
                    righttop[...,1] = xorshift(righttop[...,1]) % grid.shape[-1]
                    leftbottom[...,0] = xorshift(leftbottom[...,0]) % grid.shape[-2]
                    leftbottom[...,1] = xorshift(leftbottom[...,1]) % grid.shape[-1]
                    rightbottom[...,0] = xorshift(rightbottom[...,0]) % grid.shape[-2]
                    rightbottom[...,1] = xorshift(rightbottom[...,1]) % grid.shape[-1]

                    lefttop = grid[..., lefttop[...,0], lefttop[...,1]]
                    righttop = grid[..., righttop[...,0], righttop[...,1]]
                    leftbottom = grid[..., leftbottom[...,0], leftbottom[...,1]]
                    rightbottom = grid[..., rightbottom[...,0], rightbottom[...,1]]
                    interp = (1 - alpha_x) * (1 - alpha_y) * lefttop + \
                             (1 - alpha_x) * alpha_y * leftbottom + \
                             alpha_x * (1 - alpha_y) * righttop + \
                             alpha_x * alpha_y * rightbottom
        ...
        return outputs.reshape(*original_shape[:-1], self.num_components * self.num_levels)

    def density_L1(self):
        total = 0
        for li in range(self.num_levels):
            for ci, coo_comb in enumerate(self.coo_combs):
                total = total + torch.mean(torch.abs(self.plane_coefs_mipmap[li][ci]))
        return total
